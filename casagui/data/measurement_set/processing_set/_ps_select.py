...
def select_ms(ps_xdt, logger, indexers=None, method=None, tolerance=None, drop=False, **indexers_kwargs):
    ...
    for name, ms_xdt in ps_xdt.items():
        ...
        try:
            ...
            if dim_selection and 'frequency' in dim_selection:
                # Add selected frequencies to list
                try:
                    selected_frequencies.extend(ms_xdt.frequency.values.tolist())
                except TypeError:
                    selected_frequencies.append(ms_xdt.frequency.values.item())

            if time_selection:
                ms_xdt, success = _select_time(ms_xdt, time_selection, method, tolerance, drop)
                if success:
                    # Add selected timestrings to list (convert from float to datetime string)
                    selected_times.extend(_get_ms_selected_times(ms_xdt))
    ...
    if selected_frequencies:
        logger.debug("Selected frequencies: %s", sorted(list(set(selected_frequencies))))
    ...
    for key, val in selection.items():
        if key == 'time':
            ...
            if isinstance(val, str):
                ...
                raise TypeError(f"Time selection {time_sel} must be string, list, or slice")
